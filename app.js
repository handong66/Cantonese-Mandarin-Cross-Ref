document.addEventListener('DOMContentLoaded', () => {
    ...
    const tc2sc = {
        '請': '请', '選擇': '选择', '左側': '左侧', '右側': '右侧', '查詢': '查询', '查詢': '查询', '漢字': '汉字',
        '暫無數據': '暂无数据', '未找到漢字': '未找到汉字', '粵語': '粤语', '普通話': '普通话', '聲母': '声母',
        '韻母': '韵母', '聲調': '声调', '聲': '声', '韻': '韵', '調': '调', '對應': '对应', '顯示': '显示', '繁體中文': '繁体中文',
        '簡體中文': '简体中文', '陰平': '阴平', '陰上': '阴上', '陰去': '阴去', '陽平': '阳平', '陽上': '阳上', '陽去': '阳去',
        '佔比約': '占比约', '暫無': '暂无', '請選擇': '请选择', '以查看其與': '以查看其与', '的對應關係': '的对应关系',
        '查漢字，例如：福': '查汉字，例如：福', '查詢': '查询'
    };
    ...
    function renderLists() {
        ...
        initialsTitle.textContent = tText(isC2M ? '粵語聲母' : '普通話聲母');
        finalsTitle.textContent = tText(isC2M ? '粵語韻母' : '普通話韻母');
        // tones title (粵語 or 普通話 depending on direction)
        const tonesTitle = document.getElementById('tones-title');
        if (tonesTitle) tonesTitle.textContent = tText(isC2M ? '粵語聲調' : '普通話聲調');

        // tone-note paragraph show only for c2m (hide for 普通話查粵語)
            ...
            if (ev.key === 'Enter') {
                ...
                if (v) {
                    ...
                    if (res) {
                        ...
                        contentDisplay.innerHTML = translateHTML(`<div class="welcome-message">未找到漢字：${v}</div>`);
                    }
                }
            }
        });
    ...
    window.switchDirection = function(direction) {
        ...
        renderLists();
        // Reset view
        contentDisplay.innerHTML = translateHTML(`<div class="welcome-message">請選擇${currentDirection === 'c2m' ? '粵語' : '普通話'}${currentTab === 'initials' ? '聲母' : '韻母'}以查看其與${currentDirection === 'c2m' ? '普通話' : '粵語'}${currentTab === 'initials' ? '聲母' : '韻母'}的對應關係。</div>`);
    }
    ...
    window.switchTab = function(tabName) {
        ...
        document.querySelectorAll('.tab-btn').forEach(btn => {
            if (btn.textContent === (tabName === 'initials' ? '聲母' : tabName === 'finals' ? '韻母' : '聲調')) {
        ...
        const itemLabel = tabName === 'initials' ? '聲母' : tabName === 'finals' ? '韻母' : '聲調';
        const leftLang = currentDirection === 'c2m' ? '粵語' : '普通話';
        const rightLang = currentDirection === 'c2m' ? '普通話' : '粵語';
        contentDisplay.innerHTML = translateHTML(`<div class="welcome-message">請選擇${leftLang}${itemLabel}以查看其與${rightLang}${itemLabel}的對應關係。</div>`);
    }
    ...
    function loadData(type, key) {
        ...
        if (!data) {
            contentDisplay.innerHTML = translateHTML('<div class="welcome-message">暫無數據</div>');
            return;
        }
        ...
        const sourceLang = type === 'tones' ? (isC2M ? '粵語' : '普通話') : (isC2M ? '粵語' : '普通話');
        const targetLang = type === 'tones' ? (isC2M ? '普通話' : '粵語') : (isC2M ? '普通話' : '粵語');
        const itemType = type === 'initials' ? '聲母' : type === 'finals' ? '韻母' : '聲調';
            ...
            html += `
                <div class="correspondence-group">
                    <div class="mandarin-initial-header">
                        對應${targetLang}${itemType}: <span style="color: #AA3333;">${targetValue}</span>
                    </div>
                    <div class="char-list">
                        ${group.chars.map(charData => {
                            ...
                            <div class="char-item">
                                ...
                                <button class="audio-btn" onclick="event.stopPropagation(); playJyutpingAudio('${jyut}')" title="播放粵拼">🔊</button>
                            </div>
                        `}).join('')}
                    </div>
                    <div class="stats">
                        佔比約: ${group.percentage}%
                    </div>
                </div>
            `;
        });
    ...
    window.showCharDetail = function(char, jyutping, pinyin) {
        ...
        modalBody.innerHTML = translateHTML(`
            ...
            <div class="detail-info">
                <span>粵拼: ${jyut}</span>
                <button class="modal-audio-btn" onclick="event.stopPropagation(); playJyutpingAudio('${jyut}')" title="播放粵拼">🔊</button>
            </div>
            <div class="detail-info">普通話: ${pinyin}</div>
        `);
